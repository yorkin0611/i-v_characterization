...
VJ1_interp = interp1d(V1, J1, kind="cubic", assume_sorted=False) # In principle, this pairs voltage and current density J back into (x,y) coordinate pair to define a graph line from which characteristic values (Voc, Jsc, etc.) can be extracted.
VJ2_interp = interp1d(V2, J2, kind="cubic", assume_sorted=False) 
JV2_interp = interp1d(J2, V2, kind="cubic", assume_sorted=False) # Same thing, but coupling as (y,x) pairs instead.

# Package "scipy.optimize.newton" is used  
Voc_init = JV2_interp(0)
Jsc_init = VJ2_interp(0)
...
Voc = nwt(VJ2_interp, Voc_init, fprime=None, args=(), tol=1e-08, maxiter=2000, fprime2=None)
Jsc = -nwt(JV2_interp, Jsc_init, fprime=None, args=(), tol=1e-08, maxiter=2000, fprime2=None)

#Estimating Vmax, Pmax & FF
Pwr2 = V2*J2
Pcspline = interp1d(V2, Pwr2, kind="cubic", assume_sorted=False )
Vmax = fminbound(Pcspline, 0, Voc)
Pmax = Vmax*VJ2_interp(Vmax)
