...
V1_1dspline = interp1d(V1, J1, kind="linear", assume_sorted=False) #1d interpolated dark
V2_1dspline = interp1d(V2, J2, kind="linear", assume_sorted=False) #1d interpolated light (V,J)
J2_1dspline = interp1d(J2, V2, kind="linear", assume_sorted=False) #1d interpolated light (J,V)

Voc_init1 = J2_1dspline(0)
Jsc_init1 = V2_1dspline(0)
print("Initial guesses by scipy-interp1d_linear:")
print("Voc initial guess",Voc_init1, "V")
print("Jsc initial guess",Jsc_init1, "mA/cm2")
print()

# Group 2, cubic spline interpolation
Vcspline1 = interp1d(V1, J1, kind="cubic", assume_sorted=False) # In principle, this pairs voltage and current density J back into (x,y) coordinate pair to define a graph line from which characteristic values (Voc, Jsc, etc.) can be extracted.
Vcspline2 = interp1d(V2, J2, kind="cubic", assume_sorted=False) 
Jcspline2 = interp1d(J2, V2, kind="cubic", assume_sorted=False) # Same thing, but coupling as (y,x) pairs instead.

Voc_init2 = Jcspline2(0)
Jsc_init2 = Vcspline2(0)

print("Initial guesses by scipy-interp1d_cubic:")
...
P_1dspline = interp1d(V2, Pwr2, kind="linear", assume_sorted=False)
